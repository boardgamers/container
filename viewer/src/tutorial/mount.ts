...
export const mountTutorial: TutorialMount = async (target, { chapter, onProgress }) => {
    ...
    function renderSummary(snapshot: TutorialSnapshot<LessonState>) {
        ...
        if (!['supply-chain', 'buildings'].includes(lesson.id)) summary.append(values);
