...
export function createBoardThumbnail(root: Element) {
    ...
    async function render(
        ...
        if (source instanceof SVGElement) {
            ...
            const box =
                source instanceof SVGSVGElement ? source.viewBox.baseVal : (source as SVGGraphicsElement).getBBox();
            if (!box.width || !box.height) return null;
            content.setAttribute('viewBox', `${box.x} ${box.y} ${box.width} ${box.height}`);
            if (source === svg) {
        ...
        if (!(content instanceof SVGSVGElement)) {
            ...
            content.style.transform = `translate(${(size.width - box.width * scale) / 2}px, ${
